...
from aletheia.adapters.base import Adapter
from aletheia.adapters.types.history import History
from aletheia.adapters.types.image import try_expand_image_tags
from aletheia.adapters.utils import format_field_value, get_annotation_name, parse_value, serialize_for_json
from aletheia.clients.lm import LM
from aletheia.signatures.signature import Signature, SignatureMeta
from aletheia.signatures.utils import get_aletheia_field_type
...
def format_fields(role: str, fields_with_values: Dict[FieldInfoWithName, Any]) -> str:
    """
    Formats the values of the specified fields according to the field's aletheia type (input or output),
...
def format_turn(
    ...
    so that it can instruct an LLM to generate responses conforming to the specified aletheia signature.

    Args:
        signature: The aletheia signature to which future LLM responses should conform.
        values: A dictionary mapping field names (from the aletheia signature) to corresponding values
...
def prepare_instructions(signature: SignatureMeta):
    ...
    def field_metadata(field_name, field_info):
        ...
        if get_aletheia_field_type(field_info) == "input" or type_ is str:
            desc = ""
        elif type_ is bool:
...
def _get_structured_outputs_response_format(signature: SignatureMeta) -> pydantic.BaseModel:
    ...
    an LM request, based on the output fields of the specified aletheia signature.

    Args:
        signature: The aletheia signature for which to obtain the `response_format` request parameter.
    Returns:
    ...
    def filter_json_schema_extra(field_name: str, field_info: FieldInfo) -> FieldInfo:
        """
        Recursively filter the `json_schema_extra` of a FieldInfo to exclude aletheia internal attributes
        (e.g. `__aletheia_field_type`) and remove descriptions that are placeholders for the field name.
        """
        ...
        if field_copy.json_schema_extra:
            field_copy.json_schema_extra = {
                ...
                if key not in ("desc", "__aletheia_field_type")
            }
    ...
    return create_model("aletheiaProgramOutputs", **output_pydantic_fields)
