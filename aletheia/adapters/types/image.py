...
class Image(pydantic.BaseModel):
    ...
    def serialize_model(self):
        return "<aletheia_IMAGE_START>" + self.url + "<aletheia_IMAGE_END>"
...
def try_expand_image_tags(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for message in messages:
        ...
        if "content" in message and "<aletheia_IMAGE_START>" in message["content"]:
...
def expand_image_tags(text: str) -> Union[str, List[Dict[str, Any]]]:
    ...
    """
    image_tag_regex = r'"?<aletheia_IMAGE_START>(.*?)<aletheia_IMAGE_END>"?'
