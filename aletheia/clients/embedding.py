...
class Embedder:
    ...
    Examples:
        ...
        embedder = aletheia.Embedder("openai/text-embedding-3-small", batch_size=100)
        ...
        embedder = aletheia.Embedder(model.encode)
        ...
        embedder = aletheia.Embedder(my_embedder)
        embeddings = embedder(["hello", "world"], batch_size=1)
    ...
    def __call__(self, inputs, batch_size=None, caching=None, **kwargs):
        ...
        for batch_inputs in chunk(inputs, batch_size):
            if isinstance(self.model, str):
                ...
                raise ValueError(
                    f"`model` in `aletheia.Embedder` must be a string or a callable, but got {type(self.model)}."
                )
