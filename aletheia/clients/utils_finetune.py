...
from aletheia.utils.caching import aletheia_CACHEDIR
...
def infer_data_format(adapter: Adapter) -> str:
    if isinstance(adapter, aletheia.ChatAdapter):
        return TrainDataFormat.CHAT
    raise ValueError(f"Could not infer the data format for: {adapter}")


def get_finetune_directory() -> str:
    default_finetunedir = os.path.join(aletheia_CACHEDIR, "finetune")
    finetune_dir = os.environ.get("aletheia_FINETUNEDIR") or default_finetunedir
