...
class DataLoader(Dataset):
    ...
    def from_huggingface(
        ...
        **kwargs,
    ) -> Union[Mapping[str, List[aletheia.Example]], List[aletheia.Example]]:
        ...
        try:
            ...
            for split_name in dataset.keys():
                if fields:
                    returned_split[split_name] = [
                        aletheia.Example({field: row[field] for field in fields}).with_inputs(*input_keys)
                        for row in dataset[split_name]
                    ]
                else:
                    returned_split[split_name] = [
                        aletheia.Example({field: row[field] for field in row.keys()}).with_inputs(*input_keys)
                        for row in dataset[split_name]
                    ]
            ...
            if fields:
                return [
                    aletheia.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for row in dataset
                ]
            else:
                return [
                    aletheia.Example({field: row[field] for field in row.keys()}).with_inputs(*input_keys)
    ...
    def from_csv(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[aletheia.Example]:
        from datasets import load_dataset
        ...
        return [aletheia.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for row in dataset]
    ...
    def from_pandas(
        ...
        input_keys: tuple[str] = (),
    ) -> list[aletheia.Example]:
        if fields is None:
            fields = list(df.columns)

        return [
            aletheia.Example({field: row[field] for field in fields}).with_inputs(*input_keys) for _, row in df.iterrows()
        ]

    def from_json(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[aletheia.Example]:
        from datasets import load_dataset
    ...
    def from_parquet(self, file_path: str, fields: List[str] = None, input_keys: Tuple[str] = ()) -> List[aletheia.Example]:
        from datasets import load_dataset
    ...
    def from_rm(self, num_samples: int, fields: List[str], input_keys: List[str]) -> List[aletheia.Example]:
        try:
            rm = aletheia.settings.rm
            try:
                return [
                    aletheia.Example({field: row[field] for field in fields}).with_inputs(*input_keys)
            ...
            raise ValueError(
                "Retrieval module not found. Please set a retrieval module using `aletheia.settings.configure`."
            )

    def sample(
        self,
        dataset: List[aletheia.Example],
        n: int,
        *args,
        **kwargs,
    ) -> List[aletheia.Example]:
        if not isinstance(dataset, list):
            raise ValueError(
                f"Invalid dataset provided of type {type(dataset)}. Please provide a list of `aletheia.Example`s."
            )
    ...
    ) -> Mapping[str, List[aletheia.Example]]:
