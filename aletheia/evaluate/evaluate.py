...
import aletheia
from aletheia.utils.callback import with_callbacks
...
class Evaluate:
    """aletheia Evaluate class.

    This class is used to evaluate the performance of a aletheia program. Users need to provide a evaluation dataset and 
    a metric function in order to use this class. This class supports parallel evaluation on the provided dataset.
    """
    def __init__(
        self,
        *,
        devset: List["aletheia.Example"],
        ...
        **kwargs,
        ...
        """
        Args:
            devset (List[aletheia.Example]): the evaluation dataset.
            ...
            return_outputs (bool): Whether to return the aletheia program's outputs for every data in `devset`.
    ...
    def __call__(
        ...
        metric: Optional[Callable] = None,
        devset: Optional[List["aletheia.Example"]] = None,
        ...
        """
        Args:
            program (aletheia.Module): The aletheia program to evaluate.
            metric (Callable): The metric function to use for evaluation. if not provided, use `self.metric`.
            devset (List[aletheia.Example]): the evaluation dataset. if not provided, use `self.devset`.
            ...
            return_outputs (bool): Whether to return the aletheia program's outputs for every data in `devset`. if not
        ...
        def process_item(example):
            ...
            if hasattr(program, "_assert_failures"):
                program._assert_failures += aletheia.settings.get("assert_failures")
            if hasattr(program, "_suggest_failures"):
                program._suggest_failures += aletheia.settings.get("suggest_failures")
        ...
        results = [((aletheia.Prediction(), self.failure_score) if r is None else r) for r in results]
    ...
    def _construct_result_table(self, results: list[Tuple[aletheia.Example, aletheia.Example, Any]], metric_name: str) -> "pd.DataFrame":
        """
