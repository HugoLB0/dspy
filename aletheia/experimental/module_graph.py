...
class ModuleGraph:
    def __init__(self, module_name, module):
        ...
        self.module = module
        self.inspect_settings(aletheia.settings)
    ...
    def add_module(self, module_name, module):
        ...
        if 'aletheia.predict' in str(module_type):
    ...
    def process_submodules(self, module_name, module):
        ...
        for sub_module_name, sub_module in module.__dict__.items():
            if isinstance(sub_module, aletheia.Predict):
            ...
            elif isinstance(sub_module, (aletheia.Module, aletheia.Retrieve)):
                self.add_module(f"{module_name}__{sub_module_name}", sub_module)
                if isinstance(sub_module, aletheia.Retrieve):
...
# from aletheia.experimental import ModuleGraph

# load_dotenv()

# # Configuration of aletheia models
# llm = aletheia.OpenAI(
#     model='gpt-3.5-turbo',
...
# colbertv2_wiki = aletheia.ColBERTv2(url='http://20.102.90.50:2017/wiki17_abstracts')

# aletheia.settings.configure(lm=llm, rm=colbertv2_wiki)

# class GenerateAnswer(aletheia.Signature):
#   "Answer with long and detailed answers"
#   context = aletheia.InputField(desc="may content relevant facts")
#   question = aletheia.InputField()
#   answer = aletheia.OutputField(desc="often between 10 and 50 words")

# class RAG(aletheia.Module):
#   def __init__(self, num_passages=3):
#     super().__init__()
#     self.retrieve = aletheia.Retrieve(k=num_passages)
#     self.generate_answer = aletheia.ChainOfThought(GenerateAnswer)

#   def forward(self, question):
#     context = self.retrieve(question).passages
#     prediction = self.generate_answer(context=context, question=question)
#     return aletheia.Prediction(context=context, answer=prediction.answer)
