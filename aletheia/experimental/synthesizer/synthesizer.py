...
import aletheia
from aletheia.experimental.synthesizer.config import SynthesizerArguments
from aletheia.experimental.synthesizer.instruction_suffixes import (
    INPUT_GENERATION_TASK_WITH_EXAMPLES_SUFFIX,
    INPUT_GENERATION_TASK_WITH_FEEDBACK_SUFFIX,
)
from aletheia.experimental.synthesizer.signatures import (
    ExplainTask,
...
from aletheia.experimental.synthesizer.utils import format_examples


class Synthesizer:
    def __init__(self, config: SynthesizerArguments):
        self.config = config
        self.input_lm = config.input_lm_model or aletheia.settings.lm
        self.output_lm = config.output_lm_model or aletheia.settings.lm

        self.explain_task = aletheia.Predict(ExplainTask)
        self.understand_task = aletheia.Predict(UnderstandTask)
        self.get_feedback_on_generation = aletheia.Predict(GetFeedbackOnGeneration)
        self.generate_field_description = aletheia.Predict(GenerateFieldDescription)
        self.update_task_description = aletheia.Predict(UpdateTaskDescriptionBasedOnFeedback)

        self.generate_input_data = GenerateInputFieldsData
        self.generate_output_data = GenerateOutputFieldsData

    def _gather_feedback(self, examples: aletheia.Example) -> str:
    ...
    def _prepare_synthetic_data_predictors(
        ...
        for key in tqdm(input_keys, desc="Preparing Input Fields"):
            ...
            input_field = aletheia.InputField(
                prefix=f"{field_name}:",
                desc=field_description,
            )
        ...
        return aletheia.ChainOfThought(self.generate_input_data), aletheia.Predict(self.generate_output_data)

    def _get_dataset_metadata(self, ground_source: Union[List[aletheia.Example], aletheia.Signature]):
        if isinstance(ground_source, aletheia.SignatureMeta):
        ...
        elif isinstance(ground_source, list) and isinstance(ground_source[0], aletheia.Example):
    ...
    def generate(
        self,
        ground_source: Union[List[aletheia.Example], aletheia.Signature],
        num_data: int,
        batch_size: int = 1,
    ):
        ...
        for idx in trange(0, num_data, batch_size, desc="Generating Synthetic Data"):
            ...
            with aletheia.context(lm=self.input_lm):
            ...
            for kwargs in input_kwargs:
                ...
                with aletheia.context(lm=self.output_lm, temperature=iter_temperature):
                ...
                data.append(aletheia.Example(**kwargs, **output_kwargs).with_inputs(*input_keys))
    ...
    def export(self, data: List[aletheia.Example], path: str, mode: str = None, **kwargs):
