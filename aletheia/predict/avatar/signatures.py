import aletheia
from aletheia.predict.avatar.models import Action


class Actor(aletheia.Signature):
...
Note: You can opt to use no tools and provide the final answer directly. You can also one tool multiple times with different input queries if applicable."""
    ...
    goal: str = aletheia.InputField(
        prefix="Goal:",
        desc="Task to be accomplished.",
    )
    tools: list[str] = aletheia.InputField(
        prefix="Tools:",
        desc="list of tools to use",
    )
    action_1: Action = aletheia.OutputField(
        prefix="Action 1:",
