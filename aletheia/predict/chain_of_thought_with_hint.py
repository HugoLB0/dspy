...
class ChainOfThoughtWithHint(Module):
    def __init__(self, signature, rationale_type=None, **config):
        self.signature = aletheia.ensure_signature(signature)
        self.module = aletheia.ChainOfThought(signature, rationale_type=rationale_type, **config)
    ...
    def forward(self, **kwargs):
        if 'hint' in kwargs and kwargs['hint']:
            ...
            with aletheia.context(trace=[]):
                ...
                this_trace = aletheia.settings.trace[-1]

            aletheia.settings.trace.append((this_trace[0], original_kwargs, this_trace[2]))
            return pred
