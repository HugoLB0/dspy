...
from aletheia.primitives.python_interpreter import PythonInterpreter
...
class ProgramOfThought(Module):
    """
    A aletheia module that runs Python programs to solve a problem.
    ...
    lm = aletheia.LM('openai/gpt-4o-mini')
    aletheia.configure(lm=lm)
    pot = aletheia.ProgramOfThought("question -> answer")
    ...
    def __init__(self, signature: Union[str, Type[Signature]], max_iters=3):
        ...
        self.code_generate = aletheia.ChainOfThought(
            aletheia.Signature(
                self._generate_signature("generate").fields,
                self._generate_instruction("generate"),
            ),
        )
        self.code_regenerate = aletheia.ChainOfThought(
            aletheia.Signature(
                self._generate_signature("regenerate").fields,
                self._generate_instruction("regenerate"),
            ),
        )
        self.generate_answer = aletheia.ChainOfThought(
            aletheia.Signature(
                self._generate_signature("answer").fields,
                self._generate_instruction("answer"),
            ),
        )
        # It will raises exception when aletheia cannot find available deno instance by now.
    ...
    def _generate_signature(self, mode):
        ...
        fields_for_mode = {
            "generate": {
                "generated_code": aletheia.OutputField(
                    prefix="Code:",
            ...
            "answer": {
                "final_generated_code": aletheia.InputField(
                    prefix="Code:",
                    desc="python code that answers the question",
                    format=str,
                ),
                "code_output": aletheia.InputField(
                    prefix="Code Output:",
        ...
        return aletheia.Signature(signature_dict)
