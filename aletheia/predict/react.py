...
from aletheia.signatures.signature import ensure_signature
...
class ReAct(Module):
    def __init__(self, signature, tools: list[Callable], max_iters=5):
        """
        `tools` is either a list of functions, callable classes, or `aletheia.Tool` instances.
        """
        ...
        react_signature = (
            aletheia.Signature({**signature.input_fields}, "\n".join(instr))
            .append("trajectory", aletheia.InputField(), type_=str)
            .append("next_thought", aletheia.OutputField(), type_=str)
            .append("next_tool_name", aletheia.OutputField(), type_=Literal[tuple(tools.keys())])
            .append("next_tool_args", aletheia.OutputField(), type_=dict[str, Any])
        )

        fallback_signature = aletheia.Signature(
            {**signature.input_fields, **signature.output_fields},
            signature.instructions,
        ).append("trajectory", aletheia.InputField(), type_=str)

        self.tools = tools
        self.react = aletheia.Predict(react_signature)
        self.extract = aletheia.ChainOfThought(fallback_signature)

    def _format_trajectory(self, trajectory: dict[str, Any]):
        adapter = aletheia.settings.adapter or aletheia.ChatAdapter()
        trajectory_signature = aletheia.Signature(f"{', '.join(trajectory.keys())} -> x")
    ...
    def forward(self, **input_args):
        ...
        return aletheia.Prediction(trajectory=trajectory, **extract)
...
"""
Thoughts and Planned Improvements for aletheia.ReAct.
...
the formatter uses a general adapter.format_fields, the tracing of aletheia only sees the string, not the formatting logic.
...
One way to fix this is to support `format=fn` in the aletheia.InputField() for "trajectory" in the signatures. But this
