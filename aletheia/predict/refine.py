...
import aletheia
from aletheia.adapters.chat_adapter import enumerate_fields
from aletheia.predict.predict import Prediction
from aletheia.signatures import InputField, OutputField, Signature
...
class Refine(Module):
    def __init__(
        ...
        Example:
            ...
            qa = aletheia.ChainOfThought("question -> answer")
            ...
            best_of_3 = aletheia.Refine(module=qa, N=3, reward_fn=one_word_answer, threshold=1.0)
    ...
    def forward(self, **kwargs):
        lm = self.module.get_lm() or aletheia.settings.lm
            ...
            try:
                with aletheia.context(trace=[]):
                    if not advice:
                        ...
                        with aletheia.context(adapter=WrapperAdapter()):
                            outputs = mod(**kwargs)

                    trace = aletheia.settings.trace.copy()
                ...
                advice = aletheia.Predict(OfferFeedback)(**advise_kwargs).advice
                ...
                self.fail_count -= 1
        if best_trace:
            aletheia.settings.trace.extend(best_trace)
        return best_pred
