class Example:
    ...
    def __len__(self):
        return len([k for k in self._store if not k.startswith("aletheia_")])

    def __repr__(self):
        # return f"Example({self._store})" + f" (input_keys={self._input_keys}, demos={self._demos})"
        d = {k: v for k, v in self._store.items() if not k.startswith("aletheia_")}
    ...
    def keys(self, include_aletheia=False):
        return [k for k in self._store.keys() if not k.startswith("aletheia_") or include_aletheia]

    def values(self, include_aletheia=False):
        return [v for k, v in self._store.items() if not k.startswith("aletheia_") or include_aletheia]

    def items(self, include_aletheia=False):
        return [(k, v) for k, v in self._store.items() if not k.startswith("aletheia_") or include_aletheia]
