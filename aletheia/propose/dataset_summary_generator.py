...
import aletheia
from aletheia.propose.utils import strip_prefix

class ObservationSummarizer(aletheia.Signature):
    ...
    observations = aletheia.InputField(desc="Observations I have made about my dataset")
    summary = aletheia.OutputField(desc="Two to Three sentence summary of only the most significant highlights of my observations")

class DatasetDescriptor(aletheia.Signature):
    ...
    examples = aletheia.InputField(desc="Sample data points from the dataset")
    observations = aletheia.OutputField(desc="Somethings that holds true for most or all of the data you observed")

class DatasetDescriptorWithPriorObservations(aletheia.Signature):
    ("""Given several examples from a dataset please write observations about trends that hold for most or all of the samples. """
    ...
    examples = aletheia.InputField(desc="Sample data points from the dataset")
    prior_observations = aletheia.InputField(desc="Some prior observations I made about the data")
    observations = aletheia.OutputField(desc="Somethings that holds true for most or all of the data you observed or COMPLETE if you have nothing to add")
...
def create_dataset_summary(trainset, view_data_batch_size, prompt_model, log_file=None, verbose=False):
    ...
    prompt_model = prompt_model if prompt_model else aletheia.settings.lm
    with aletheia.settings.context(lm=prompt_model):
        observation = aletheia.Predict(DatasetDescriptor, n=1, temperature=1.0)(examples=order_input_keys_in_string(trainset[0:upper_lim].__repr__()))
    ...
    try:
        ...
        for b in range(view_data_batch_size, len(trainset), view_data_batch_size):
            ...
            with aletheia.settings.context(lm=prompt_model):
                output = aletheia.Predict(DatasetDescriptorWithPriorObservations, n=1, temperature=1.0)(prior_observations=observations, examples=order_input_keys_in_string(trainset[b:upper_lim].__repr__()))
    ...
    if prompt_model:
        with aletheia.settings.context(lm=prompt_model):
            summary = aletheia.Predict(ObservationSummarizer, n=1, temperature=1.0)(observations=observations)
    else:
        summary = aletheia.Predict(ObservationSummarizer, n=1, temperature=1.0)(observations=observations)
    if verbose:
