...
import aletheia
from aletheia.propose.dataset_summary_generator import create_dataset_summary
from aletheia.propose.utils import create_example_string, create_predictor_level_history_string, strip_prefix, get_aletheia_source_code
from aletheia.teleprompt.utils import get_signature, get_prompt_model
...
class DescribeProgram(aletheia.Signature):
    (
...
class DescribeModule(aletheia.Signature):
    (
...
def generate_instruction_class(
    ...
    class GenerateSingleModuleInstruction(aletheia.Signature):
        (
            """Use the information below to learn about a task that we are trying to solve using calls to an LM, then generate a new instruction that will be used to prompt a Language Model to better solve the task."""
        )
        if use_dataset_summary:
            dataset_description = aletheia.InputField(
...
class GenerateModuleInstruction(aletheia.Module):
    def __init__(
        ...
        self.describe_program = aletheia.Predict(DescribeProgram)
        self.describe_module = aletheia.Predict(DescribeModule)
    ...
    def forward(
        ...
        if self.program_aware:
            try:
                ...
                for field_name, field in get_signature(program.predictors()[pred_i]).fields.items():
                    # Access the '__aletheia_field_type' from the extra metadata
                    aletheia_field_type = field.json_schema_extra.get('__aletheia_field_type')
                    
                    # Based on the '__aletheia_field_type', append to the respective list
                    if aletheia_field_type == "input":
        ...
        return aletheia.Prediction(proposed_instruction=proposed_instruction)
...
class GroundedProposer(Proposer):
    def __init__(
        ...
        if self.program_aware:
            try:
                self.program_code_string = get_aletheia_source_code(program)
                if self.verbose:
    ...
    def propose_instruction_for_predictor(
        ...
        with aletheia.settings.context(lm=self.prompt_model):
