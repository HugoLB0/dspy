...
from aletheia.teleprompt.utils import get_signature, new_getfile
...
def get_aletheia_source_code(module):
    header = []
    base_code = ""

    # Don't get source code for Predict or ChainOfThought modules (NOTE we will need to extend this list as more aletheia.modules are added)
    ...
    for attribute in module.__dict__.keys():
        ...
        for item in iterable:
            ...
            if isinstance(item, aletheia.Module):
                code = get_aletheia_source_code(item).strip()
