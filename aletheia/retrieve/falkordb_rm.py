...
try:
    ...
    raise ImportError(
        "Please install the falkordb package by running `pip install aletheia-ai[falkordb]`"
    )
...
class FalkordbRM(Retrieve):
    ...
    Returns a list of passages in the form of `aletheia.Prediction` objects
    ...
    Examples:
        ...
        aletheia.settings.configure(rm=falkordb_retriever)
        ```
