...
try:
    ...
    raise ImportError(
        "The 'pgvector' extra is required to use PgVectorRM. Install it with `pip install aletheia-ai[pgvector]`. Also, try `pip install pgvector psycopg2`.",
    )
try:
    import openai
except ImportError:
...
class PgVectorRM(aletheia.Retrieve):
    """
    ...
    Returns a list of aletheia.Example objects
        ...
        with self.conn as conn:
            with conn.cursor() as cur:
                ...
                for row in rows:
                    ...
                    retrieved_docs.append(aletheia.Example(**data))
        # Return Prediction
