...
try:
    ...
    raise ImportError(
        "The 'weaviate' extra is required to use WeaviateRM. Install it with `pip install aletheia-ai[weaviate]`",
    ) from err


class WeaviateRM(aletheia.Retrieve):
    ...
    Examples:
        ...
        llm = aletheia.Cohere(model="command-r-plus", api_key=api_key)
        ...
        retrieve = aletheia.Retrieve(k=1)
