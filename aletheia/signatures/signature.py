...
from aletheia.adapters.types.image import Image  # noqa: F401
from aletheia.signatures.field import InputField, OutputField
...
class SignatureMeta(type(BaseModel)):
    ...
    def _validate_fields(cls):
        for name, field in cls.model_fields.items():
            ...
            field_type = extra.get("__aletheia_field_type")
    ...
    def _get_fields_with_type(cls, field_type) -> dict[str, FieldInfo]:
        return {k: v for k, v in cls.model_fields.items() if v.json_schema_extra["__aletheia_field_type"] == field_type}
...
class Signature(BaseModel, metaclass=SignatureMeta):
    ...
    def insert(cls, index: int, name: str, field, type_: Type = None) -> Type["Signature"]:
        ...
        lst = input_fields if field.json_schema_extra["__aletheia_field_type"] == "input" else output_fields
        ...
        if index < 0 or index > len(lst):
            raise ValueError(
                ...
                f"{field.json_schema_extra['__aletheia_field_type']} fields, but received: {index}.",
            )
...
def _parse_type_node(node, names=None) -> Any:
    ...
    def resolve_name(type_name: str):
        ...
        # This allows handling of module-based annotations like `aletheia.Image`.
        try:
    ...
    raise ValueError(
        ...
        "Please consider using class-based aletheia Signatures instead."
    )
