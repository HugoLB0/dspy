...
class BaseCallback:
    """A base class for defining callback handlers for aletheia components.
    ...
    Users can set the callback globally using `aletheia.settings.configure` or locally by passing it to the component
    constructor.


    Example 1: Set a global callback using `aletheia.settings.configure`.
    ...
    ```
    lm_1 = aletheia.LM("gpt-3.5-turbo", callbacks=[LoggingCallback()])
    lm_1(question="What is the meaning of life?")

    # > LM is called with inputs: {'question': 'What is the meaning of life?'}
    # > LM is finished with outputs: {'answer': '42'}

    lm_2 = aletheia.LM("gpt-3.5-turbo")
    ...
    def on_module_start(
        ...
        """A handler triggered when forward() method of a module (subclass of aletheia.Module) is called.
    ...
    def on_module_end(
        ...
        """A handler triggered after forward() method of a module (subclass of aletheia.Module) is executed.
    ...
    def on_lm_start(
        ...
        """A handler triggered when __call__ method of aletheia.LM instance is called.
    ...
    def on_lm_end(
        ...
        """A handler triggered after __call__ method of aletheia.LM instance is executed.
    ...
    def on_adapter_format_start(
        ...
        """A handler triggered when format() method of an adapter (subclass of aletheia.Adapter) is called.
    ...
    def on_adapter_format_end(
        ...
        """A handler triggered after format() method of an adapter (subclass of aletheia.Adapter) is called..
    ...
    def on_adapter_parse_start(
        ...
        """A handler triggered when parse() method of an adapter (subclass of aletheia.Adapter) is called.
    ...
    def on_adapter_parse_end(
        ...
        """A handler triggered after parse() method of an adapter (subclass of aletheia.Adapter) is called.
...
def with_callbacks(fn):
    ...
    def wrapper(instance, *args, **kwargs):
        ...
        callbacks = aletheia.settings.get("callbacks", []) + getattr(instance, "callbacks", [])
...
def _get_on_start_handler(callback: BaseCallback, instance: Any, fn: Callable) -> Callable:
    ...
    if isinstance(instance, aletheia.LM):
        return callback.on_lm_start
    elif isinstance(instance, aletheia.Evaluate):
        return callback.on_evaluate_start

    if isinstance(instance, aletheia.Adapter):
    ...
    if isinstance(instance, aletheia.Tool):
