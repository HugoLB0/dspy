...
from aletheia.adapters.chat_adapter import FieldInfoWithName, field_header_pattern, format_fields
...
class DummyLM(LM):
    ...
    aletheia.settings.configure(lm=lm)
    predictor("What color is the sky?, demos=aletheia.Example(input="What color is the sky?", output="blue"))
    # Output:
    # [[## answer ##]]
    # blue
    ```
...
def dummy_rm(passages=()) -> callable:
    ...
    def inner(query: str, *, k: int, **kwargs):
        ...
        # return aletheia.Prediction(passages=[passages[i] for i in largest_idx])
