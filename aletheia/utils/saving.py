...
def get_dependency_versions():
    ...
    return {
        ...
        "aletheia": aletheia.__version__,
        "cloudpickle": cloudpickle_version,
    }


def load(path):
    """Load saved aletheia model.

    This method is used to load a saved aletheia model with `save_program=True`, i.e., the model is saved with cloudpickle.

    Args:
        path (str): Path to the saved model.

    Returns:
        The loaded model, a `aletheia.Module` instance.
    """
    path = Path(path)
    if not path.exists():
